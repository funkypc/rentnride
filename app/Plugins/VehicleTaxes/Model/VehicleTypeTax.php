...
namespace Plugins\VehicleTaxes\Model;
...
class VehicleTypeTax extends Model
{
    /**
     * @var string
     */
    protected $table = 'vehicle_type_taxes';

    protected $fillable = [
        'vehicle_type_id', 'tax_id', 'rate', 'discount_type_id', 'duration_type_id', 'max_allowed_amount', 'is_active',
    ];

    /**
     * @return \Illuminate\Database\Eloquent\Relations\BelongsTo
     */
    public function vehicle_tax()
    {
        return $this->belongsTo(VehicleTax::class, 'tax_id');
    }
    ...
    public function scopeFilterByRequest($query, Request $request)
    {
        ...
        if ($request->has('q')) {
            $query->WhereHas('vehicle_tax', function ($q) use ($request) {
    ...
    public function scopeGetValidationMessage()
    {
        return [
            ...
            'tax_id.integer' => 'Tax Id must be integer',
            'tax_id.exists' => 'Invalid tax id',
