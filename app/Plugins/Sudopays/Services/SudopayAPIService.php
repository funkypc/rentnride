...
class SudopayAPIService
{
    ...
    private function _doPost($url, $post = [])
    {
        return $this->_execute($url, 'post', $post);
    }

    private function _doPut($url, $post = [])
    {
    ...
    private function _createSignature($post = [])
    {
        $query_string = '';
    ...
    private function _execute($url, $method = 'get', $post = [])
    {
        $ch = curl_init();
