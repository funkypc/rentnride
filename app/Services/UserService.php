...
class UserService extends MailService
{
    ...
    public function emailConditions($user, $type)
    {
        ...
        } elseif (config('user.is_email_verification_for_register') && ($user->is_email_confirmed == 0)) {
        ...
        } elseif (config('user.is_admin_activate_after_register') && ($user->is_active == 0) && ($type == 'activate')) {
        ...
        } elseif (! config('user.is_admin_activate_after_register') && config('user.is_auto_login_after_register') && ($type == 'activate')) {
        ...
        } elseif (! config('user.is_email_verification_for_register') && ! config('user.is_admin_activate_after_register') && config('user.is_welcome_mail_after_register') && ! config('user.is_auto_login_after_register')) {
            $this->sendWelcomeMail($user->id, $user->email, $user->username);

            return response()->json(['Success' => 'You have successfully registered with our site.'], 200);
        } elseif (! config('user.is_email_verification_for_register') && ! config('user.is_admin_activate_after_register') && config('user.is_auto_login_after_register') && config('user.is_welcome_mail_after_register')) {
            $this->sendWelcomeMail($user->id, $user->email, $user->username);

            return response()->json(['Success' => 'You have successfully registered with our site.', 'token' => $this->tokenService->createToken($user)], 200);
        } elseif (config('user.is_auto_login_after_register') && config('user.is_welcome_mail_after_register')) {
            $this->sendWelcomeMail($user->id, $user->email, $user->username);
