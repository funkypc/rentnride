(function (module) {
     ...
     **/
    module.controller('UserRegisterController', function ($auth, $state, Flash, $rootScope, $filter, AuthFactory, $location, gRecaptcha, $scope) {
        var model = this;
        /**
         * @ngdoc method
         * @name setMetaData
        ...
        model.init = function () {
            ...
            // model.captcha_secret_key = $rootScope.settings['captcha.secret_key'];
        ...
        model.signup = function (isvalid) {
            ...
            gRecaptcha.execute({action: 'signupform'})
            .then(function (token) {
                $scope.model.grecapresp = token;
                /* OUTPUT: {"name": "test test", "email": "test@test.com", "bodymsg": "testing", "grecapresp": "03AOLTBLTFACwWq[truncated]"} */
                //{"captcha_site_key":"6LemKO8kAAAA...","captchaErr":"","grecapresp":"03AFY_a8U-mohl4rKC....","username":"us....","nameErr":"","password":"pass...","passwordErr":"","confirmPasswordErr":""}
                console.log(token);
                console.log($scope.model.grecapresp);
                if(!token || token.length === 0){
                    model.captchaErr = $filter("translate")("Please resolve the captcha and submit");
                }else{
                    model.captchaErr = '';
                }
                // var recaptcha_url = 'https://www.google.com/recaptcha/api/siteverify?secret=' + $scope.model.captcha_secret_key + '&response=' + token;
                // $http.post(recaptcha_url).then(function (response) {
                //     if (response.data){
                ...
                if (isvalid && model.password == model.confirm_password) {
                    ...
                    $auth.signup(credentials).then(function (response) {
                        ...
                        var captchaError = error.data.message;
                        if (captchaError) {
                            console.log(captchaError);
                            model.captchaErr = $filter("translate")(captchaError);
                        }
